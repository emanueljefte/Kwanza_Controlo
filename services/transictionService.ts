...
export const deleteTransaction = async (
  ...
  try {
    // 1. Buscar a transação ANTES de deletar para saber o que reverter
    const [transactionData] = await db
      .select()
      .from(schema.transactions)
      .where(eq(schema.transactions.id, transactionId));
    ...
    const { type, amount, walletId: tWalletId } = transactionData;

    // 2. Buscar a carteira
    const [walletData] = await db
      .select()
      .from(schema.wallets)
      .where(eq(schema.wallets.id, tWalletId as number));

    if (!walletData) return { success: false, msg: "Carteira não encontrada" };

    // 3. Calcular novos valores com lógica clara
    let newAmount = Number(walletData.amount);
    let newTotalIncome = Number(walletData.totalIncome);
    let newTotalExpenses = Number(walletData.totalExpenses);

    if (type === "income") {
      newAmount -= Number(amount); // Remove o ganho do saldo
      newTotalIncome -= Number(amount); // Remove do acumulado de ganhos
    } else {
      newAmount += Number(amount); // Devolve o gasto ao saldo
      newTotalExpenses -= Number(amount); // Remove do acumulado de gastos
    }

    if (newAmount < 0) {
      return {
        success: false,
        msg: "Não podes eliminar esta transação porque a carteira ficaria com saldo negativo.",
      };
    }

    // 4. Update da Carteira (Atómico)
    await db
      .update(schema.wallets)
      .set({
        amount: newAmount,
    ...
    // 5. Lógica de Deleção (Online vs Offline)
    const net = await NetInfo.fetch();
    if (net.isConnected) {
      // Tenta deletar no servidor...
      // Se conseguir, deleta local. Se não, marca dirty.
      // (Mantém a tua lógica de fetch aqui)
      await db
    ...
    return { success: true, msg: "Transação removida e saldo atualizado" };
  } catch (error: any) {
    return { success: false, msg: error.message };
  }
};
...
export const fetchMonthlyStats = async (uid: string): Promise<ResponseType> => {
  try {
    ...
    const transactions: any[] = []; // Ajusta para o teu tipo que inclui walletName
    ...
    transationsQuery.forEach((doc) => {
      ...
      const cleanMonthName = monthName.replace(".", "").toLowerCase();
      const transactionMonthLabel = `${cleanMonthName} ${shortYear}`;

      // 3. Comparar com o monthlyData também normalizado
      const monthData = monthlyData.find((m) => {
        const cleanLabel = m.month.replace(".", "").toLowerCase();
        return cleanLabel === transactionMonthLabel;
      });

      if (monthData) {
        if (doc.type === "income") monthData.income += Number(doc.amount);
        else if (doc.type === "expense")
          monthData.expense += Number(doc.amount);
      }

      if (doc.type === "expense" && doc.category) {
        categoryTotals[doc.category] =
          (categoryTotals[doc.category] || 0) + Number(doc.amount);
      }
    });

    // Formatação do stats para o Chart
