...
export const AuthProvider = ({children}: {children: ReactNode}) => {
    ...
    const login = async (email: string, password: string) => {
        try {
            ...
            if (msg.includes("(auth/network-request-failed)")) msg = "Sem conexão com a internet. Tente novamente mais tarde!"
    ...
    const register = async (email: string, password: string, name: string) => {
        try {
            ...
            let msg = error.message
            if (msg.includes("(auth/email-already-in-use)")) msg = "Este e-mail já se encontra em uso"
            if (msg.includes("(auth/invalid-email)")) msg = "E-mail Inválido"
            if (msg.includes("(auth/network-request-failed)")) msg = "Sem conexão com a internet. Tente novamente mais tarde!"
            return {success: false, msg}
        }
